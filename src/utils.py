...
def import_query_traverse(name):
    ...
    # import all available image descriptors
    descriptors_dir = os.path.join(query_path, params.traverses[name],
...
def import_reference_map(name):
    # import timestamps
    tstamps_dir = os.path.join(reference_path, params.traverses[name],
                               "stereo_tstamps.npy")
    tstamps = np.load(tstamps_dir)
    # import RTK GPS poses for images
    rtk_dir = os.path.join(reference_path, params.traverses[name],
                           "rtk/stereo/left/rtk.pickle")
    with open(rtk_dir, 'rb') as f:
        rtk = pickle.load(f)
    rtk_poses = rtk['rtk']
    # import all available image descriptors
    descriptors_dir = os.path.join(reference_path,
...
def localize_traverses_filter(model, query_descriptors, gt=None,
                              vo=None, desc=None):
...
def localize_traverses_matching(model, query_poses, query_descriptors,
                                desc=None, idx=0):
    nloc = len(query_descriptors)  # number of localizations
    # localize trials!
...
def remove_far_queries(ref_tree, ref_poses, query_poses):
    ...
    for i in range(len(query_poses)):
        ...
        dist, ind = ref_tree.nearest(query_seq.t(),
                                     query_seq.R().as_quat(), 1, 1)
        divergence.append(np.sum(dist))

    divergence = np.asarray(divergence)
    ind_not_outlier = np.squeeze(np.argwhere(divergence <
