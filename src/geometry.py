...
class SE3Poses:
    def __init__(self, t, R):
        ...
        if t.ndim not in [1, 2] or t.shape[-1] != 3:
            raise ValueError("Expected `t` to have shape (3,) or"
                             "(N x 3), got {}.".format(t.shape))

        # If a single quaternion is given, convert it to a 2D 
        # 1 x 4 matrix but set self._single to True so that we 
        # can return appropriate objects in the `to_...` methods
        if t.shape == (3,):
            t = t[None, :]
            self._single = True
            if len(R) > 1: 
                raise ValueError("Different number of translations 1"
                                 "and rotations {}.".format(len(R)))
        elif len(t) == 1:
            self._single = True
        else:
            if len(t) != len(R):
    ...
    def __truediv__(self, other):
        ...
        TO DO: Broadcasting
        """
        if not(len(self) == 1 or len(other) == 1 or len(self)
               == len(other)):
            raise ValueError("Expected equal number of transformations"
                                "in both or a single transformation in"
                                "either object, got {} transformations"
