...
class TopologicalFilter:
    def __init__(self, map_poses, map_descriptors,
                 delta, window_lower=-2, window_upper=10):
        # store map data
    ...
    def obs_lhood(self, descriptor):
        vsim = np.exp(-self.lambda1 *
                      np.sqrt(2 - 2 * np.dot(self.map_descriptors,
                                             descriptor)))
        return vsim
    ...
    def update(self, descriptor):
        ...
        if w_l < 0:
            conv_ind_l, conv_ind_h = np.abs(w_l),
            len(self.belief) + np.abs(w_l)
        ...
        self.belief[bel_ind_l:bel_ind_h] = np.convolve(
            self.belief, self.transition)[conv_ind_l:conv_ind_h]
        if w_l > 0:
            self.belief[:w_l] = 0.0
    ...
    def localize(self):
        ...
        proposal = self.map_poses[
            int(np.rint(np.average(nhood_inds,
                                   weights=self.belief[nhood_inds])))]
    ...
    for traverse in pbar:
        ...
        for desc in pbar:
            ...
            model = TopologicalFilter(
                ref_poses, ref_descriptors[desc], args.delta,
                window_lower=args.window_lower,
                window_upper=args.window_upper)
            proposals, scores, times = \
                utils.localize_traverses_filter(
                    model, query_descriptors[desc], vo=None,
                    desc='Topological')
            utils.save_obj(save_path1 + '/Topological.pickle',
                           model='Topological', query_gt=query_poses,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run topological"
                                     "filter on trials")
    ...
    parser.add_argument('-D', '--delta', type=float, default=5,
                        help="multiple used for calibrating sensor"
    ...
    parser.add_argument('-wl', '--window-lower', type=int, default=-2, 
                        help="minimum state transition in transition"
                        "matrix")
    parser.add_argument('-wu', '--window-upper', type=int, default=10,
                        help="maximum state transition in transition"
                        "matrix")
