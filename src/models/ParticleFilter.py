...
class ParticleFilter:
    ...
    def initialize_model(self, query_descriptor):
        dists = np.linalg.norm(
            self.descriptors - query_descriptor[np.newaxis, :], axis=1
        )
        ...
        map_idx = np.minimum(
            self._resample_systematic(sims), len(self.map_poses) - 1
        )  # resample map indices
    ...
    def apply_measurement(self, particles, query_descriptor):
        ...
        u_gt_ind, inv_ind = np.unique(gt_ind, return_inverse=True)
        ...
        if not self.odom_only:
            ...
            desc_dist = np.linalg.norm(
                self.descriptors[u_gt_ind] - query_descriptor[np.newaxis, :], axis=1
            )
            # visual similarity (alpha)
            visual_sim = self._visual_sim(desc_dist)
            update = np.sum(
                pose_wts * visual_sim[inv_ind].reshape(gt_dist.shape), axis=1
            )
        else:
    ...
    def _resample_systematic(self, weights):
        u = (
            np.random.uniform(0, 1) + np.arange(0, self.n_particles, 1)
        ) / self.n_particles
        ...
        if not args.regular_only:
            ...
            for desc in pbar:
                ...
                for _ in trange(args.ntrials, leave=False, desc="Trials"):
                    ...
                    proposals, scores, times = utils.localize_traverses_filter(
                        model, query_descriptors[desc], vo=rtk_motion, desc="RTK motion"
                    )
                    proposals_all.append(proposals)
                    scores_all.append(scores)
                    times_all.append(times)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Names of query traverses to localize against"
    ...
    parser.add_argument(
        "-N", "--ntrials", type=int, default=5, help="number of trials to run"
    )
    parser.add_argument(
        "-M",
        "--nparticles",
    ...
    parser.add_argument(
        ...
        default=5,
        help=(
            "multiple used for calibrating sensor update"
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "number of nearest neighbours keyframes for"
    ...
    parser.add_argument(
        ...
        "--auxiliary",
        action="store_true",
        help="use auxiliary particle filter insteadof bootstrap",
    )
    parser.add_argument(
        "-g",
        "--gt-motion",
    ...
    parser.add_argument(
        "-o", "--odom-only", action="store_true", help="use odometry component only"
    )
    parser.add_argument(
        "-re", "--regular-only", action="store_true", help="regular vo only"
    )
