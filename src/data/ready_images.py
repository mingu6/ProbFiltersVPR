...
def ready_images(traverse, camera, n_cpus=4, resume=True):
    # load and undistort images
    image_folder_path = os.path.join(RAW_PATH, traverse, camera)
    cm = camera_model.CameraModel(RAW_PATH + '/camera-models/', image_folder_path) # this is a variable used in process_and_save_image
    try:
    ...
    if resume and os.path.exists(ready_folder_path):
        ...
        img_fnames = [img_path for img_path in os.listdir(image_folder_path) if img_path.endswith(".png") and img_path not in ready_fnames]
    else:
    ...
    try:
        ...
        with Pool(n_cpus) as pool:
            list(tqdm(pool.imap(process_and_save_image, zip(full_raw_path, full_ready_path, itertools.repeat(cm))), total=len(full_raw_path)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--n-cpu', type=int, default=8, help="Number of CPUs to use")
    parser.add_argument('-r', '--resume', action='store_true', help="Skip images already undistored in traverse/camera combinations")
    args = parser.parse_args()

    for traverse in params.values():
