...
def process_raw_traverse(name):
    ...
    vo_cumulative = interpolate_poses.interpolate_vo_poses(vo_path, 
            img_tstamps, img_tstamps[0])
    vo_cumulative = np.asarray(vo_cumulative)
    vo = geometry.SE3Poses(vo_cumulative[:, :3, 3], Rotation.from_dcm(vo_cumulative[:, :3, :3]))
    ...
    rel_rtk_poses = interpolate_poses.interpolate_ins_poses(rtk_path, 
            img_tstamps, img_tstamps[0], use_rtk=True)
    ...
    rtk_interp = geometry.SE3Poses(rtk_mat[:, :3, 3], Rotation.from_dcm(rtk_mat[:, :3, :3]))
...
if __name__ == "__main__":
    ...
    for name in pbar:
        ...
        base_dir = os.path.join(utils.processed_path, params.traverses[name])
