...
def main(args):
    ...
    results_path_0 = os.path.join(utils.results_path,
                                  args.query_traverses[0],
                                  args.descriptors[0])
    ...
    for desc in args.descriptors:
        ...
        fig, axs = plt.subplots(nQueries, nThres, figsize=(6 * nThres, 
                                                           3.5 * nQueries))
        ...
        for i in range(nQueries):
            results_path = os.path.join(utils.results_path,
                                        args.query_traverses[i], desc)
            for j, (t_thres, R_thres) in enumerate(zip(args.t_thres,
                                                       args.R_thres)):
                # initialize PR curve plots
                axs[i, j].set_aspect(0.5)
                if i == 0:
                    axs[i, j].set_title('{:.0f}m, {:.0f}deg'.format(
                        t_thres, R_thres), fontsize=20)
                ...
                for k, fname in enumerate(
                    ...
                    with open(os.path.join(results_path, fname), 'rb') \
                            as f:
                    ...
                    if model in ['Seq Match', 'Single', 'Topological']:
                        PR = evaluate.PRCurve(t_thres, R_thres *
                                              np.pi / 180, model)
                        PR.generate_curve(query_poses, proposals, scores)
                        axs[i, j].plot(PR.recalls, PR.precisions,
                                       label=model,
                                       linestyle=linestyle[model],
                                       color=colors[model], linewidth=3)
                    else:
                        PRs = []
                        for l, (p, s) in enumerate(
                            ...
                            PRs.append(PR)
                        PR_mean, PR_min, PR_max = evaluate.max_min_avg_PR(
                            PRs)
                        axs[i, j].plot(PR_mean.recalls,
                                       PR_mean.precisions,
                                       label=model,
                                       linestyle=linestyle[model],
                                       color=colors[model], linewidth=3)
                        axs[i, j].fill_between(PR_mean.recalls,
                                               PR_min.precisions,
                                               PR_max.precisions,
                    ...
                    if model in ['Seq Match', 'Single']:
                        ...
                        if model == 'Topological':
                            score_at, recall_at = \
                                PR.score_precision_level(
                                    args.precision_level)
                            step_localized = \
                                PR._localize_indices(scores, score_at) 
                            compute_at_localized = \
                                np.squeeze(
                                    np.take_along_axis(
                                        np.asarray(times),
                                        step_localized[:, np.newaxis], 1)
                                )
                        else:
                            score_at, recall_at = \
                                PR_mean.score_precision_level(
                                    args.precision_level)
                            step_localized = \
                                PR_mean._localize_indices(scores[0],
                                                          score_at)
                            compute_at_localized = \
                                np.squeeze(np.take_along_axis(
                                    np.asarray(times[0]),
                                    step_localized[:, np.newaxis], 1)
                                           )
                        total_compute = \
                            compute_at_localized[step_localized != -1]
                        nsteps = step_localized[step_localized != -1] + 1
                    if model in ['Single', 'Seq Match', 'Topological']:
                        AUC_results[i, j, k] = PR.auc()
                    else:
                        AUC_results[i, j, k] = PR_mean.auc()
                    max_recall_results[i, j, k] = recall_at
                    compute_results[i, j, k] = \
                        np.mean(total_compute * 1000 / nsteps)
                    nsteps_results[i, j, k] = np.mean(nsteps)
                    # side headings for traverse name
                    if j == 0:
                        axs[i, j].text(-0.22, 0.40,
        ...
        plt.savefig(utils.figures_path + '/PR_{}.png'.format(desc), 
                    bbox_inches='tight')
        ...
        for i in range(nModels):
            ...
            table_maxrecall.append([
                models[i], *max_recall_results[..., i].reshape(-1).
                tolist()
            ])
            table_AUC.append([
                models[i], *AUC_results[..., i].reshape(-1).tolist()
            ])
        ...
        for traverse in args.query_traverses:
            for t_thres, R_thres in zip(args.t_thres, args.R_thres):
                headers.append("{} {}m, {}deg".format(
                    traverse, t_thres, R_thres
                ))
        fname_out = os.path.join(
            utils.figures_path, 'table_{}.txt'.format(desc)
        )
        ...
        print("\nAUC\n\n", tabulate(table_AUC, floatfmt=".3f",
                                    headers=headers),
              file=open(fname_out, "a"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run sequence"
                                     "matching on trials")
    parser.add_argument('-q', '--query-traverses', nargs='+',
                        type=str, default=['Rain', 'Dusk', 'Night'],
                        help="Names of query traverses to plot results"
    ...
    parser.add_argument('-t', '--t-thres', type=float,
                        nargs='+', default=[3, 5],
                        help="maximum error for translation distance (m) \
                        for a correct localization")
    parser.add_argument('-R', '--R-thres', type=float,
                        nargs='+', default=[15, 30],
                        help="maximum error for angle (degrees) for a \
                        correct localization")
    parser.add_argument('-p', '--precision-level', type=float,
                        default=0.99,
                        help="Precision level to evaluate table"
                        "elements at")
