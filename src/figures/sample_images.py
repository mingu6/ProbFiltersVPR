    ...
    for i in range(nRows):
        ...
        for j in range(nCols):
            if i == 0 and j == 0:
                ...
                axs[i, j].set_title("{:.0f}m {:.0f} deg".format(t_tol[j-1], R_tol[j-1]), fontsize=32)
            if j == 0:
                imgFolder = os.path.join(utils.raw_path, params.traverses[args.reference], 'stereo/left')
                ...
                indQ = np.squeeze(bothOK[np.argmax(SE3dist[bothOK])]) # maximum distance query within thresholds
                # display image
                imgFolder = os.path.join(utils.raw_path, params.traverses[traverse], 'stereo/left')
                imgPath = os.path.join(imgFolder, str(query_tstamps[indQ]) + '.png')
            # import camera model to undistort images
            camera = camera_model.CameraModel(utils.raw_path + 'camera-models/', imgFolder)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-q', '--queries', nargs='+', type=str, default=['Rain', 'Dusk', 'Night'],
